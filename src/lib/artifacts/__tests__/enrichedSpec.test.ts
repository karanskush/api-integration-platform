...
describe('buildEnrichedSpec', () => {
  ...
  it('tags a produced-by-api field with its origin and known producer', () => {
    ...
    expect(petId['x-docentapi-origin']).toBe('produced_by_api');
    expect(petId['x-docentapi-produced-by'].length).toBeGreaterThan(0);
    expect(petId['x-docentapi-human-verified']).toBe(false);
  });
  ...
  it('marks a field as human-verified when it appears in the verified set', () => {
    ...
    expect(petId['x-docentapi-human-verified']).toBe(true);
  });

  // The whole point of asking. Before this, an answer only flipped
  // human-verified to true while x-docentapi-origin stayed heuristic — so an
  ...
  describe('a human answer outranks the heuristic', () => {
    ...
    it('rewrites the origin and says the answer came from a human', () => {
      ...
      expect(before['x-docentapi-origin']).toBe('produced_by_api');
      expect(before['x-docentapi-origin-source']).toBe('heuristic');

      const after = petIdOf({ answers: new Map([['get_pet path.petId', { origin: 'caller_supplied' as const }]]) });
      expect(after['x-docentapi-origin']).toBe('caller_supplied');
      expect(after['x-docentapi-origin-source']).toBe('human');
      expect(after['x-docentapi-human-verified']).toBe(true);
    });
    ...
    it('still marks a field verified when the answer resolved to no origin', () => {
      ...
      expect(field['x-docentapi-human-verified']).toBe(true);
      expect(field['x-docentapi-origin']).toBe('produced_by_api'); // unchanged
      expect(field['x-docentapi-origin-source']).toBe('heuristic');
    });
    ...
    it('records a skipped question as unresolved rather than silently dropping it', () => {
      ...
      expect(asked['x-docentapi-unresolved']).toBe(true);
      expect(asked['x-docentapi-human-verified']).toBe(false);

      // Never asked is not the same as asked-and-unanswerable.
      expect(petIdOf()['x-docentapi-unresolved']).toBeUndefined();
    });

    it('withholds a disputed producer and reclassifies what remains', () => {
      const before = petIdOf();
      const producers: Array<{ operation: string; field: string }> = before['x-docentapi-produced-by'];
      expect(producers.length).toBeGreaterThan(0);

      const all = new Set(producers.map((p) => `get_pet path.petId ${p.operation}.${p.field}`));
      const after = petIdOf({ disputed: all });
      expect(after['x-docentapi-produced-by']).toBeUndefined();
      // With every producer withheld, produced_by_api no longer holds.
      expect(after['x-docentapi-origin']).not.toBe('produced_by_api');
    });
    ...
    it('applies an assumption but never lets it claim a human confirmed it', () => {
      ...
      expect(field['x-docentapi-origin']).toBe('caller_supplied');
      expect(field['x-docentapi-origin-source']).toBe('assumed');
      // The line that matters: evidence is not confirmation.
      expect(field['x-docentapi-human-verified']).toBe(false);
      // And it carries its receipt, so a consumer can judge the inference.
      expect(field['x-docentapi-assumed']).toEqual({
        quote: 'callers supply the pet identifier themselves',
        source: 'docs',
    ...
    it('lets a person override an assumption about the same field', () => {
      ...
      expect(field['x-docentapi-origin']).toBe('server_generated');
      expect(field['x-docentapi-origin-source']).toBe('human');
      expect(field['x-docentapi-human-verified']).toBe(true);
      expect(field['x-docentapi-assumed']).toBeUndefined();
    });
  ...
  it('carries operationId, description, and safety on each operation entry', () => {
    ...
    expect(createPet['x-docentapi-safety']).toBe('write');
  });
});
