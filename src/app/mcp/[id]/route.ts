...
async function handler(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const mcp = createMcpHandler(
    ...
    },
    {
      ...
      capabilities: { tools: {} },
      instructions: [
        `Tools for the "${record.name}" API, generated by Spotcheck from its ${record.source} spec (${exposed.length} operations exposed).`,
        `Start with spotcheck_search_endpoints to find the right operation instead of reading every tool schema.`,
        `Before calling any write operation, call spotcheck_describe_fields to see exactly what you may send — it labels each field's origin (caller-supplied, another operation's response, an enum, or server-assigned and must not be sent).`,
        `Before calling any operation whose path OR body contains an identifier, call spotcheck_get_call_sequence — it shows which operation produces that identifier — or spotcheck_trace_field for the same question about one specific field, in either direction. Do not invent identifiers.`,
